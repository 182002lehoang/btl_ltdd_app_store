...
const Accessory = () => {
    ...
    const [products, setProducts] = React.useState<Product3[]>([]);

    // Lưu dữ liệu vào AsyncStorage (chỉ chạy 1 lần nếu chưa có dữ liệu)
    React.useEffect(() => {
        const saveDataToLocal = async () => {
            const existingData = await AsyncStorage.getItem('Product3');
            if (!existingData) {
                await AsyncStorage.setItem('Product3', JSON.stringify(Product3));
            }
        };
        saveDataToLocal();
    }, []);
    ...
    React.useEffect(() => {
        const loadDataFromLocal = async () => {
            const storedData = await AsyncStorage.getItem('Product3');
            //console.log(Product3);
            console.log('Dữ liệu trong AsyncStorage3:', JSON.parse(storedData || '[]'));
            if (storedData) {
    ...
    const renderProduct = ({ item }: { item: Product3 }) => (
        <Pressable
    ...
    return (
        <SafeAreaView style={styles.container}>
            <Text style={styles.title}>Phu kien </Text>
            <FlatList
                data={products}
