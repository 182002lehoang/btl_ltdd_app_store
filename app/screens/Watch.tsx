...
const Watch = () => {
    ...
    const [products, setProducts] = React.useState<Product4[]>([]);

    // Lưu dữ liệu vào AsyncStorage (chỉ chạy 1 lần nếu chưa có dữ liệu)
    React.useEffect(() => {
        const saveDataToLocal = async () => {
            const existingData = await AsyncStorage.getItem('Product4');
            if (!existingData) {
                await AsyncStorage.setItem('Product4', JSON.stringify(Product4));
            }
        };
        saveDataToLocal();
    }, []);
    ...
    React.useEffect(() => {
        const loadDataFromLocal = async () => {
            const storedData = await AsyncStorage.getItem('Product4');
    ...
    const renderProduct = ({ item }: { item: Product4 }) => (
        <Pressable
    ...
    return (
        <SafeAreaView style={styles.container}>
            <Text style={styles.title}>dong ho</Text>
            <FlatList
                data={products}
